...
namespace Loxodon.Framework.Bundles
{
    ...
    public class SceneLoadingResult<TResult> : ISceneLoadingResult<TResult>, ISceneLoadingPromise<TResult>
    {
        ...
        {
            return new SceneLoadingResultAwaiter(this);
        }
#endif
...
#if NETFX_CORE || NET_STANDARD_2_0 || NET_4_6
        public struct SceneLoadingResultAwaiter : IAwaiter, ICriticalNotifyCompletion
        {
            private SceneLoadingResult<TResult> result;

            public SceneLoadingResultAwaiter(SceneLoadingResult<TResult> result)
            {
                this.result = result ?? throw new ArgumentNullException("result");
            }

            public bool IsCompleted => result.IsDone;
            ...
            public void UnsafeOnCompleted(Action continuation)
            {
                ...
                {
                    if (state == LoadState.Completed || state == LoadState.Failed)
                    {
                        continuation();
                    }
                });
