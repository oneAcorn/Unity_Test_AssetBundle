...
#pragma warning disable 0414, 0219 
    public class ObjectArchive : AssetArchive
    {
        ...
        protected virtual void Load(ArchiveBinaryReader reader)
        {
            try
            {
                ...
                long fileSize = reader.ReadInt32(true);
                ...
                if(Format >= 22)
                {
                    headerSize = reader.ReadInt32(true);
                    fileSize = reader.ReadInt64(true);
                    assetDataOffset = reader.ReadInt64(true);
                    reader.ReadInt64();
                }
                ...
                for (var i = 0; i < typeTreeCount; i++)
                {
                    ...
                    var scriptID = typeId == 114 ? reader.ReadHash128() : zero;
                    var typeHash = reader.ReadHash128();
                    var tree = new TypeTree(this, i, Enum.IsDefined(typeof(TypeID), typeId) ? (TypeID)typeId : TypeID.UnknownType,isStrippedType, scriptIndex, typeHash, scriptID);
                    tree.Load(reader);

                    if (Format >= 21)
                        tree.TypeDependencies = reader.ReadInt32Array();
                ...
                for (var i = 0; i < objectCount; i++)
                {
                    ...
                    var offset = Format>=22 ? reader.ReadInt64(): reader.ReadInt32();
    ...
    public class ObjectItem
    {
        ...
        public ObjectItem(long id, long offset, int size, TypeTree typeTree)
        {
            this.ID = id;
            this.Offset = offset;
            this.Size = size;
