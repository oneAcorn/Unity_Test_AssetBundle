...
namespace Loxodon.Framework.Bundles.Archives
{
    ...
    public class TypeNode
    {
        ...
        public ulong RefTypeHash { get; internal set; }
    ...
    public class TypeTree : TypeNode
    {
        ...
        public Hash128 TypeHash { get; private set; }

        public Hash128 ScriptID { get; private set; }

        public int[] TypeDependencies { get; set; }

        public ObjectArchive Archive { get; private set; }

        public TypeTree(ObjectArchive archive, int id, TypeID typeId, bool isScrippedType, short scriptIndex, Hash128 hash, Hash128 scriptID) : base(null)
        {
            this.Root = this;
            this.Archive = archive;
            this.ID = id;
            this.TypeID = typeId;
            this.IsStrippedType = isScrippedType;
        ...
        {
            var format = this.Archive.Format;
            var nodeCount = reader.ReadInt32();
            var typeTableBufferSize = reader.ReadInt32();

            int sizeOf = format >= 19 ? 32 : 24;
            var nodeBuffer = reader.ReadBytes(sizeOf * nodeCount);
            ...
            using (var nodeReader = new ArchiveBinaryReader(new MemoryStream(nodeBuffer)))
            {
                ...
                int position = nodeReader.ReadInt32();
                this.TypeName = (position & 0x80000000) == 0 ? stringTable.GetString(position) : baseStringTable.GetString(position & 0x7FFFFFFF);
                ...
                position = nodeReader.ReadInt32();
                this.FieldName = (position & 0x80000000) == 0 ? stringTable.GetString(position) : baseStringTable.GetString(position & 0x7FFFFFFF);

                this.Size = nodeReader.ReadInt32();
                this.Index = nodeReader.ReadInt32();
                this.Flags = nodeReader.ReadInt32();
                this.RefTypeHash = format >= 19 ? nodeReader.ReadUInt64() : 0;
                ...
                for (var i = 1; i < nodeCount; i++)
                {
                    ...
                    position = nodeReader.ReadInt32();
                    current.TypeName = (position & 0x80000000) == 0 ? stringTable.GetString(position) : baseStringTable.GetString(position & 0x7FFFFFFF);

                    //position = nodeReader.ReadUInt16();
                    //flag = nodeReader.ReadUInt16();
                    //current.FieldName = flag == 0 ? stringTable.GetString(position) : baseStringTable.GetString(position);
                    position = nodeReader.ReadInt32();
                    current.FieldName = (position & 0x80000000) == 0 ? stringTable.GetString(position) : baseStringTable.GetString(position & 0x7FFFFFFF);

                    current.Size = nodeReader.ReadInt32();
                    current.Index = nodeReader.ReadInt32();
                    current.Flags = nodeReader.ReadInt32();
                    current.RefTypeHash = format >= 19 ? nodeReader.ReadUInt64() : 0;
        ...
        internal override void ToString(StringBuilder buf, int rootDepth)
        {
            ...
            buf.AppendFormat("Flags:{0}, ", this.Flags);
            buf.AppendFormat("IsAlign:{0}, ", this.IsAlign);
            buf.AppendFormat("TypeHash:{0}, ", this.TypeHash);
            if (this.ScriptID != zero)
                buf.AppendFormat("ScriptID:{0}, ", this.ScriptID);
