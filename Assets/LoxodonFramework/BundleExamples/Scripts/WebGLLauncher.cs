...
using RijndaelCryptograph = Loxodon.Framework.Security.Cryptography.RijndaelCryptograph;
    ...
    public class WebGLLauncher : MonoBehaviour
    {
        ...
        IResources CreateResources(BundleManifest manifest)
        {
...
#endif
            {
                ...
                /* AES128_CBC_PKCS7 */
                RijndaelCryptograph cryptograph = new RijndaelCryptograph(128, Encoding.ASCII.GetBytes(this.key), Encoding.ASCII.GetBytes(this.iv));

                /* AES128_CTR_NONE */
                //AesCTRCryptograph cryptograph = new AesCTRCryptograph(Encoding.ASCII.GetBytes(this.key), Encoding.ASCII.GetBytes(this.iv));

                /* Use a custom BundleLoaderBuilder */
                ILoaderBuilder builder = new CustomBundleLoaderBuilder(new Uri(BundleUtil.GetReadOnlyDirectory()), false, cryptograph);
